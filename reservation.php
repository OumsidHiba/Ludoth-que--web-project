...
if ($_SERVER["REQUEST_METHOD"] === "POST" && empty($erreurs)) {
    ...
    if ($dateReservation === "") {
    ...
    } elseif ($dateReservation < $aujourdhui) {
        $erreurs[] = "La date de réservation ne peut pas être dans le passé.";
    } elseif (date("N", strtotime($dateReservation)) != 4) {
        $erreurs[] = "Les réservations sont uniquement possibles le jeudi.";
    } else {
        $stmt = $pdo->prepare("
            SELECT COUNT(*) 
            FROM reservation
            WHERE id_jeu = :id_jeu
              ...
              AND LOWER(statut) IN ('en attente', 'validée')
        ");
        $stmt->execute([
            "id_jeu" => $idJeu,
        ...
        $dejaReserve = (int)$stmt->fetchColumn() > 0;

        if ($dejaReserve) {
            $erreurs[] = "Ce jeu est déjà réservé pour cette date.";
        } else {
            $stmt = $pdo->prepare("
